...
Hooks.once("init", async function () {
  ...
  game.settings.register("mega", "retraitAuto", {
    ...
    hint: "Si coché, les dêgats, points de vie, d'ardence et résonnance seront retirés automatiquement",
    scope: "world",
    config: true,
    default: true,
    type: Boolean,
  ...
  game.settings.register("mega", "bagarre_video_path", {
    ...
    type: String,
    default:
      "modules/videos/Generic/Unarmed_Attacks/Flurry_Of_Blows/FlurryOfBlows_01_Regular_Blue_Physical01_800x600.webm",
  });
  ...
  game.settings.register("mega", "bagarre_son_path", {
    ...
    config: true,
    type: String,
    default: "systems/mega/assets/sounds/Effects/punch.mp3",
  });
  ...
  game.settings.register("mega", "charge_video_path", {
    ...
    type: String,
    default:
      "modules/videos/2nd_Level/Gust_Of_Wind/GustOfWind_01_White_VeryFast_1200x200.webm",
  });
  ...
  game.settings.register("mega", "charge_son_path", {
    ...
    config: true,
    type: String,
    default: "systems/mega/assets/sounds/Effects/charge.mp3",
  });
  ...
  game.settings.register("mega", "pouvoir_video_path", {
    ...
    config: true,
    type: String,
    default: "systems/mega/assets/videos/pouvoir.webm",
  });
  ...
  game.settings.register("mega", "pouvoir_son_path", {
    ...
    config: true,
    type: String,
    default: "systems/mega/assets/sounds/Effects/psi.mp3",
  });
...
Hooks.on("renderSettingsConfig", (app, html, data) => {
  ...
  if (megaSettings.length > 0) {
    ...
    megaSettings.each(function () {
      ...
      if (settingKey && settingKey.startsWith("mega.")) {
        ...
        if (
          ...
          newGroup = "interface";
        } else if (settingKey.includes("bagarre")) {
          newGroup = "bagarre";
        } else if (settingKey.includes("charge")) {
          newGroup = "charge";
        } else if (settingKey.includes("pouvoir")) {
        ...
        if (newGroup && newGroup !== currentGroup) {
          ...
          switch (newGroup) {
            ...
            case "charge":
              ...
              break;
            case "pouvoir":
              groupTitle = "Pouvoirs Psychiques";
              break;
          }

          // Créer le séparateur de titre
          const separator =
          ...
          // Pour les groupes "general" et "interface", créer un conteneur avec encadré
          if (newGroup === "general" || newGroup === "interface") {
            ...
            $setting.before(separator);
            // Les paramètres de combat gardent leur ordre naturel après leurs séparateurs
          }

          currentGroup = newGroup;
        } else if (currentGroup === "general" || currentGroup === "interface") {
          // Ajouter les éléments suivants au conteneur du groupe
    ...
    // Réorganiser l'ordre : déplacer le groupe Interface juste après le groupe Paramètres Généraux
    if (megaSettings.length > 0) {
      const $form = megaSettings.first().closest("form");
      if ($form.length > 0) {
        ...
        // Si on trouve les deux groupes, déplacer Interface après Général
        if ($generalContainer.length > 0 && $interfaceSeparator.length > 0) {
          // Déplacer le séparateur Interface et son conteneur après le conteneur Général
