// Ce fichier est obsolète - l'API socket est maintenant intégrée dans socket.js
// Gardé pour éviter les erreurs de référence mais ne doit plus être utilisé

export const api = {};

console.warn("Le fichier mega-socket.js est obsolète. Utilisez game.systems.get('mega').api à la place.");
});
...
function getDocument(entity, skipCheck) {
  ...
  if (document instanceof foundry.abstract.Document || skipCheck) {
    ...
    throw new Error(
      `${MODULE_ID}| Object provided was not a Document or reducible to one.`
    );
}

function getUniqueID() {
  return `${game.user.id}-${Date.now()}-${foundry.utils.randomID()}`;
}
