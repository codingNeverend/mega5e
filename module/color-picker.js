...
class Setting {
  ...
  constructor(module, key, settingOptions = {}, pickerOptions = {}) {
    ...
    this.jsColor = null;
    this.currentColor =
      this.pickerOptions.value ?? this.settingOptions.default ?? "#FFFFFFFF";
    ...
    try {
      this.currentColor = game.settings.get(module, key) || this.currentColor;
    } catch (e) {
      console.warn(`Failed to retrieve setting ${module}.${key}:`, e);
      this.currentColor = this.settingOptions.default;
    }

    Hooks.on("renderSettingsConfig", (app, html, data) => {
      // Convertir en objet jQuery si nécessaire pour la compatibilité v13
      const $html = html instanceof jQuery ? html : $(html);
      const element = $html[0].querySelector(
        `[name='${this.module}.${this.key}']`,
      );
      if (!element) return;

      // Re-retrieve the current value in case it was updated
      try {
        this.currentColor =
          game.settings.get(this.module, this.key) ||
          this.settingOptions.default;
      } catch (e) {
        this.currentColor = this.settingOptions.default;
      }

      // Replace placeholder element
      const newElement = $(element).replaceWith(
        `<input type="text" id="color-picker-${this.module}-${this.key}" name="${this.module}.${this.key}" value="${this.currentColor}">`,
      );
      if (!newElement) return;
      // Attach jscolor to element
      this.pickerOptions.value = this.currentColor;
      this.jsColor = new JsColor(
        `#color-picker-${this.module}-${this.key}`,
        this.pickerOptions,
      );

      // Set colors when 'Save Changes' button is pressed
      $(
        $html[0].querySelector("form.categories footer button[type=submit]"),
      ).on("click", () => {
      ...
      $($html[0].querySelector("aside.sidebar button.reset-all")).on(
        "click",
        () => {
